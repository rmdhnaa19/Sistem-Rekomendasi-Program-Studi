...
<div class="card">
    <div class="card-body">
        @if ($errors->any())
            ...
            @endif
        <form method="POST" action="{{ route('admin.organisasi.update', $organisasi->id_organisasi) }}" class="form-horizontal"
            ...
            <div class="form-group">
                ...
                <select name="id_kriteria" class="form-control @error('id_kriteria') is-invalid @enderror" required>
                    ...
                    @foreach($kriteria as $item)
                        <option value="{{ $item->id_kriteria }}" {{ old('id_kriteria', $organisasi->id_kriteria) == $item->id_kriteria ? 'selected' : '' }}>
                            {{ $item->nama_kriteria }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control @error('nama') is-invalid @enderror" id="nama"
                    name="nama" value="{{ old('nama', $organisasi->nama) }}" placeholder="Masukkan nama organisasi"
                    required>
            ...
            <div class="form-group">
                ...
                <input type="number" class="form-control @error('nilai') is-invalid @enderror" id="nilai"
                    name="nilai" value="{{ old('nilai', $organisasi->nilai) }}" placeholder="Masukkan nilai index"
                    required>
                <p><small class="text-muted">Wajib Diisi!</small></p>
                @if ($errors->has('nilai'))
            ...
            <div class="d-flex justify-content-between mt-4">
                <button type="button" class="btn btn-danger btn-sm" onclick="window.location.href='{{ route('admin.organisasi.index') }}'">Kembali</button>
