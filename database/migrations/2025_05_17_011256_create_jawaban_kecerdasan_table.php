...
return new class extends Migration {
    ...
    {
        if (!Schema::hasTable('jawaban_kecerdasan')) {
            Schema::create('jawaban_kecerdasan', function (Blueprint $table) {
                ...
                $table->unsignedBigInteger('id_kecerdasan_majemuk')->index(); // Foreign Key ke kecerdasan_majemuk
                $table->integer('poin'); // 1 jika YA, 0 jika TIDAK
                $table->timestamps();

                // Foreign Key Constraints
                $table->foreign('id_konsultasi')->references('id_konsultasi')->on('konsultasi');
                $table->foreign('id_pertanyaan_kecerdasan')->references('id_pertanyaan_kecerdasan')->on('pertanyaan_kecerdasan');
